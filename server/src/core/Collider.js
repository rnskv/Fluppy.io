class Collider {
  ...
  checkRectangleRectangle(rectangle1, rectangle2) {
    ...
    if (
      rectangle1.x + rectangle1.shape.size.x + rectangle1.shape.size.width >=
        rectangle2.x + rectangle2.shape.size.x &&
      rectangle1.x + rectangle1.shape.size.x <=
        rectangle2.x + rectangle2.shape.size.x + rectangle1.shape.size.width
    )
      XColl = true;
    if (
      rectangle1.y + rectangle1.shape.size.y + rectangle1.shape.size.height >=
        rectangle2.y + rectangle2.shape.size.y &&
      rectangle1.y + rectangle1.shape.size.y <=
        rectangle2.y + rectangle2.shape.size.y + rectangle1.shape.size.height
    )
      YColl = true;
  ...
  checkCircleRectangle(circle, rectangle) {
    ...
    const distX = Math.abs(
      circle.x +
        circle.shape.size.x -
        rectangle.x +
        rectangle.shape.size.x -
        rectangle.shape.width / 2
    );
    const distY = Math.abs(
      circle.y +
        circle.shape.size.y -
        rectangle.y +
        rectangle.shape.size.y -
        rectangle.shape.height / 2
    );

    if (distX > rectangle.shape.size.width / 2 + circle.shape.size.radius) {
      return false;
    }
    if (distY > rectangle.shape.size.height / 2 + circle.shape.size.radius) {
      return false;
    }

    if (distX <= rectangle.shape.size.width / 2) {
      return true;
    }
    if (distY <= rectangle.shape.size.height / 2) {
      return true;
    }
    ...
    return (
      dx * dx + dy * dy <= circle.shape.size.radius * circle.shape.size.radius
    );
  }
